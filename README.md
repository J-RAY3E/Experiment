...
Компилятор HL эмитирует строку как серию директив `.word` — первой идёт длина, затем каждый символ как его ASCII-код.
...
| `v` (вектор) | кортеж `("v", Vn)` | `VLD V0, [gp+off]` |
...
**Использование `s0..s11`:** `VREGS` (`hl_logic.py:14`) зарезервированы для переменных цикла и векторной адресации (см. `_eb()` при векторизации: `rlo`, `rro`, `rro2`, `ci`, `sl`, `sr`, `sd`). В скалярном коде эти регистры не используются для хранения переменных — все скалярные переменные размещаются в gp-относительной памяти.
...
- **Микрокодированная** CU: горизонтальная µROM с 29 сигналами.
...
- **Vector extension**: 8 векторных регистров по 4 lane × int32.
...
В варианте `vector` реализована поэлементная векторная обработка 4-lane массивов. Сравнение двух реализаций одной и той же задачи (сложение двух 4-элементных массивов `{1,2,3,4} + {10,20,30,40}`, печать результата `11 22 33 44 \n`):
...
| ------------------------------------ | ----------: | --------: |
| Тактов до `HALT` (полная программа)  | 582         | 448       |
| Ускорение (полная программа)         | 1×          | **~1.30×**|
| Выход программы                      | `11 22 33 44 \n` | `11 22 33 44 \n` |
| Тактов до `HALT` (только `c = a + b`) | 265         | 44        |
| Ускорение (только `c = a + b`)       | 1×          | **~6.02×**|

> **Почему так?** Скалярный цикл тратит ~66 тактов на элемент (адресация + LW + LW + ADD + SW + loop overhead). Векторный — ~11 тактов на элемент (VLD + VLD + VADD + VST делятся на 4 элемента). Разница в скорости обусловлена не только шириной 4×, но и меньшим числом итераций (1 вместо 4), что снижает overhead цикла.
