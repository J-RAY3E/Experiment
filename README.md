...
> **Почему так?** Скалярный цикл тратит 37 тактов на элемент (адресация + LW + LW + ADD + SW + loop overhead). Векторный — 7.5 тактов на элемент (VLD + VLD + VADD + VST делятся на 4 элемента). Разница в скорости обусловлена не только шириной 4×, но и меньшим числом итераций (1 вместо 4), что снижает overhead цикла.
