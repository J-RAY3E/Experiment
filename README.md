...
- `examples/alg1.alg` — решает Project Euler №1 (сумма кратных 3 или 5 до 999);
- `examples/alg1_optimized.alg`, `examples/alg1_bruteforce.alg` — два варианта той же задачи.
...
**Процедуры.** `function main()` — единственная точка входа.
...
* **R-type / I-type (arith):** `FETCH` → `EXECUTE` → `WRITEBACK`.
* **Load (`LW`):** `FETCH` → `ADDR_CALC` (`L_EX1`) → `MEM_ACCESS` (`L_EX2`) → `WRITEBACK` (`L_EX3`).
* **Store (`SW`):** `FETCH` → `ADDR_CALC` (`S_EX1`) → `MEM_ACCESS` (`S_EX2`).
* **Branch:** `FETCH` → `EXECUTE` (`B_EX` — сравнение и `PC+imm`).
...
| `examples/alg1.alg`  | Project Euler №1, исходное решение                                                          |
| `examples/alg1_optimized.alg`   | Project Euler №1, оптимизированное решение                                  |
| `examples/alg1_bruteforce.alg`  | Project Euler №1, brute-force решение                                      |
