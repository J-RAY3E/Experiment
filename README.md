...
|---------------------------------------------------------------------|------:|--------------------------------------------------------------------------|
| `R` (`ADD`/`SUB`/`MUL`/`DIV`/`REM`/`MULH`/`AND`/`OR`/`XOR`/`NOT`/`SLL`/`SRL`/`SRA`/`SLT`/`NOP`) | 2 | `Fetch + R_EX`                                       |
| `I` (`ADDI`/`ANDI`/`ORI`/`XORI`/`SLLI`/`SRLI`/`SRAI`/`SLTI`/`LUI`)   | 2     | `Fetch + I_EX` (или `Fetch + U_EX` для `LUI`)                          |
| `LW`/`LB`                                                           | 4     | `Fetch + L_EX1 + L_EX2 + L_EX3`                                        |
| `SW`/`SB`                                                           | 3     | `Fetch + S_EX1 + S_EX2`                                                |
| `BEQ`/`BNE`/`BLT`/`BLE`/`BGT`/`BGE`/`BGTU`/`BLEU`                   | 2     | `Fetch + B_EX`                                                          |
| `J`                                                                | 2     | `Fetch + J_EX`                                                          |
| `JAL`               | 3     | `Fetch + JL_EX1 + JL_EX2` (JL_EX1: link; JL_EX2: jump)               |
| `JR`                                                               | 2     | `Fetch + JR_EX`                                                         |
| `V` (`VADD`/`VSUB`/`VMUL`/`VDIV`/`VCMP`)                            | 2     | `Fetch + V_EX` (vector ALU считает 4 lane за 1 такт)                  |
| `VLD`                                                              | 6     | `Fetch + VLD_EX → VLD_W0 → VLD_W1 → VLD_W2 → VLD_W3` |
| `VST`                                                              | 6     | `Fetch + VST_EX1 → VST_W0 → VST_W1 → VST_W2 → VST_W3` |
...
`scalar loop` (см. `examples/testNoVector.asm`):
...
`vector` (см. `examples/testVector.asm`):
...
Архитектура **не является конвейерной** — в каждый момент времени выполняется ровно одна микростадия одной инструкции. Переход между стадиями управляется микрокодом (µROM + микросеквенсор).
...
`ControlUnit` — горизонтально-микрокодированный. µROM (`src/control_path.py:184-243`) содержит 29 сигналов. На каждом такте выбирается запись по `uPC`; после исполнения `uPC` обновляется одним из способов:
...
`FETCH` (запись 0) устанавливает `ir_we = 1` и `pc_inc = 1`, затем `br_type = 2` диспатчит по формату.

Имена µROM-стадий (28 записей, индексы 0–27):
...
Каждая микростадия — один такт модели. Даже `FETCH` занимает 1 такт.
...
| Фаза        | Назначение                                                                                                       |
|-------------|------------------------------------------------------------------------------------------------------------------|
| `Fetch`     | чтение инструкции по `PC`, запись в `IR`, инкремент `PC`                                                        |
| `R_EX`      | исполнение R-инструкции: чтение `rs1`/`rs2`, ALU, writeback                                                    |
| `I_EX`      | то же для I-инструкции с immediate                                                                              |
| `L_EX1/2/3` | вычисление адреса, чтение памяти, writeback в регистр                                                          |
| `S_EX1/2`   | вычисление адреса, запись в память                                                                              |
| `B_EX`      | вычисление `PC + imm`, запись в `feedback_bus`, условный апдейт `PC`                                          |
| `J_EX`/`JL_EX1/2`/`JR_EX`/`U_EX` | переходы и загрузка upper-immediate                                        |
| `HALT_EX`   | остановка                                                                                                        |
...
|-------------|------------------------------------------------------------------------------------------------------------------|
| `V_EX`      | vector ALU                                                                                                      |
| `VLD_EX → VLD_W0 → VLD_W1 → VLD_W2 → VLD_W3` | vector load: 6 тактов (каждый Wi совмещает `MEM[vbase]` + `VRF[rd][i]`) |
| `VST_EX1 → VST_W0 → VST_W1 → VST_W2 → VST_W3` | vector store: 6 тактов |
