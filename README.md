...
- `examples/alg1.alg` — решает Project Euler №1 (сумма кратных 3 или 5 до 100);
...
Используется **гарвардская архитектура** (вариант `harv`): память инструкций и память данных физически разделены. Машинное слово — 32 бита. Память данных — байт-адресуемая (byte-addressable), память инструкций — слов-адресуемая (word-addressable, доступ через `instruction_memory[PC >> 2]`).

Параметры памяти:

- машинное слово: 32 бита;
- инструкция: 32 бита фиксированной длины;
- память данных: байтовая адресация (byte-addressable), выравнивание 4 байта;
- размер памяти данных: 32768 байт (32 KiB, см. `DATA_MEM_SIZE` в `src/isa.py:32`);
- база стека: `0x4000` (`STACK_BASE`), стек растёт вниз;
- память инструкций (instruction memory): word-addressable, адресация: `instruction_memory[PC >> 2]`, PC — байтовый адрес;
