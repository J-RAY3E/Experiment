...
from src.ISA import (
    OPCODES, REG, R_FORMAT, I_FORMAT, L_FORMAT, S_FORMAT, B_FORMAT,
    V_FORMAT,
    OPCODE_SHIFT, RD_SHIFT, RS1_SHIFT, RS2_SHIFT,
    IMM11_MASK, IMM21_MASK, IMM26_MASK, WORD_MASK, HALT_WORD,
)
...
def encode(mnem, ops, labels, addr):
    ...
    if mnem in I_FORMAT or mnem in L_FORMAT:
        return (op << OPCODE_SHIFT) | (rnum(ops[0]) << RD_SHIFT) | (rnum(ops[1]) << RS1_SHIFT) | (imm(ops[2]) & IMM11_MASK)
    if mnem in S_FORMAT:
        return (op << OPCODE_SHIFT) | (rnum(ops[0]) << RD_SHIFT) | (rnum(ops[1]) << RS1_SHIFT) | (imm(ops[2]) & IMM11_MASK)
    if mnem in B_FORMAT:
    ...
    if mnem == "J":
        return (op << OPCODE_SHIFT) | (resolve(ops[0]) & IMM26_MASK)
    if mnem == "JAL":
        return (op << OPCODE_SHIFT) | (rnum(ops[0]) << RD_SHIFT) | (resolve(ops[1]) & IMM21_MASK)
    if mnem == "JR":
    ...
    if mnem == "VLD" or mnem == "VST":
        m2 = re.match(r"\[(\w+)\s*\+\s*([^\]]+)\]", "".join(ops[1:]))
        if m2:
            return (op << OPCODE_SHIFT) | (vnum(ops[0]) << RD_SHIFT) | (rnum(m2.group(1)) << RS1_SHIFT) | (imm(m2.group(2)) & IMM11_MASK)
        return 0
    if mnem == "HALT":
        return HALT_WORD
    ...
    for text in source.split("\n"):
        ...
        if ":" in text:
            ...
            if re.match(r"^[a-zA-Z_]\w*$", parts[0].strip()): 
                ...
                text = parts[1].strip()
        if not text:
            lines.append({"lb": lb, "m": None, "ops": []})
            continue
        m_str = re.match(r'\.string\s+(".*")\s*$', text, re.I)
        if m_str:
            lines.append({"lb": lb, "m": ".STRING", "ops": [m_str.group(1)]})
            continue
        toks = [t for t in re.split(r"[\s,]+", text) if t]
        mnem, ops = toks[0].upper(), toks[1:]
        if mnem in PSEUDO_SET:
            for e in expand(ops, mnem):
                e["lb"] = lb
                lb = None
                lines.append(e)
        else:
            lines.append({"lb": lb, "m": mnem, "ops": ops})
    labels, addr = {}, 0
    for ln in lines:
        if ln["lb"]:
            labels[ln["lb"]] = addr
